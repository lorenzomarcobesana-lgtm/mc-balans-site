...
async function main() {
  ...
  const conds = await pool.query('SELECT slug, name FROM conditions ORDER BY slug');
  conds.rows.forEach(r => console.log(`  ${r.slug}  =>  ${r.name}`));
  console.log('\n=== CATEGORIES ===');
  const cats = await pool.query('SELECT slug, name FROM categories ORDER BY slug');
  cats.rows.forEach(r => console.log(`  ${r.slug}  =>  ${r.name}`));
  console.log('\n=== TREATMENTS ===');
  const treats = await pool.query('SELECT slug, name FROM treatments ORDER BY slug');
  treats.rows.forEach(r => console.log(`  ${r.slug}  =>  ${r.name}`));
  process.exit(0);
}
main().catch(e => { console.error(e); process.exit(1); });cat > server.js << 'EOF'
