  ...
  try {
    const conditionResult = await pool.query(`
      SELECT c.*,
             ...
             COALESCE(t_approach.value, (SELECT value FROM translations WHERE shared_block_id = c.approach_block_id AND locale = $2 LIMIT 1), c.approach) AS approach,
             COALESCE(t_assessment.value, (SELECT value FROM translations WHERE shared_block_id = c.assessment_block_id AND locale = $2 LIMIT 1), c.assessment) AS assessment,
